# ==============================================================================
# Keras predRNN
# (the current implementation is derive from https://github.com/kami93/PredRNN)
#
# Reference:
...
from tensorflow.python.keras.layers.recurrent import RNN, _standardize_args
...
class STLSTM2D(RNN):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        if unroll:
            raise TypeError('Unrolling isn\'t possible with ' 'STLSTMs.')
    ...
    def compute_output_shape(self, input_shape):
        ...
        if not self.return_sequences:  # only the last output is returned
            output_shape = output_shape[:1] + output_shape[
                2:]  # [batch, <single_shape>]

        if self.return_state:  # [[outputshape], (batch, <single_shape>) for "m, hN, cN..., "h1", "c1"]
            output_shape = [output_shape]
            output_shape += [
                tuple([input_shape[0]] + single_shape)
  ...
  """
    def __init__(self, cells, **kwargs):
        if not hasattr(cells, '__len__'):
            raise ValueError('Only a list of STLSTMCells'
                             'is stackable.', 'received input:', cells)
        first = True
        for cell in cells:
            if not isinstance(cell, STLSTMCell):
                raise ValueError(
                    'All cells must be a `STLSTMCell`'
                    'received cell:', cell)
            if first:
                first = False
                continue
                ...
                cell.state_size = cell.state_size[
                    1:]  # remove m-states from non 1st cells.
    ...
    def from_config(cls, config, custom_objects=None):
        from tensorflow.python.keras.layers import \
            deserialize as \
            deserialize_layer  # pylint: disable=g-import-not-at-top
        cells = []
  ...
  """
    ...
    def call(self, inputs, states, training=None):
        ...
        x_c = self.input_conv(inputs_c,
                              ...
                              self.bias_c,
                              padding=self.padding)
        x_ip = self.input_conv(inputs_ip,
                               self.kernel_xip,
                               self.bias_ip,
                               padding=self.padding)
        x_fp = self.input_conv(inputs_fp,
                               self.kernel_xfp,
                               self.bias_fp,
                               padding=self.padding)
        x_m = self.input_conv(inputs_m,
                              self.kernel_xm,
                              self.bias_m,
                              padding=self.padding)
        ...
        h_i = self.recurrent_conv(h_tm1_i, self.kernel_hi)
        h_f = self.recurrent_conv(h_tm1_f, self.kernel_hf)
        h_c = self.recurrent_conv(h_tm1_c, self.kernel_hc)
        h_o = self.recurrent_conv(h_tm1_o, self.kernel_ho)
        m_i = self.recurrent_conv(m_tm1, self.kernel_mi)
        m_f = self.recurrent_conv(m_tm1, self.kernel_mf)
        m_m = self.recurrent_conv(m_tm1, self.kernel_mm)
        i = self.recurrent_activation(x_i + h_i)
        f = self.recurrent_activation(x_f + h_f)
        c = f * c_tm1 + i * self.activation(x_c + h_c)
        c_o = self.recurrent_conv(c, self.kernel_co)

        ip = self.recurrent_activation(x_ip + m_i)
        fp = self.recurrent_activation(x_fp + m_f)
        m = fp * m_tm1 + ip * self.activation(x_m + m_m)
        m_o = self.recurrent_conv(m, self.kernel_mo)

        cm_h = self.recurrent_conv(K.concatenate([c, m]), self.kernel_1by1)
