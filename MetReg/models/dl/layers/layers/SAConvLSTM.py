...
class SelfAttentionConvLSTM2DCell(DropoutRNNCellMixin, Layer):
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(SelfAttentionConvLSTM2DCell, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.depth_wise_kernel = self.add_weight(
            shape=depth_wise_kernel_size,
            initializer=self.kernel_initializer,
        ...
        if self.use_bias:
            self.sa_bias = self.add_weight(shape=(self.filters * 3),
                                           name='bias',
    ...
    def call(self, inputs, states, training=None):
        ...
        h_hat_t = m_t * o
        # sa computation end
        return h_hat_t, [h_hat_t,c_t, m_t]
    ...
    def get_config(self):
        ...
        base_config = super(SelfAttentionConvLSTM2DCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


class SelfAttentionConvLSTM2D(layers.Layer): 
    def __init__(self,
                 filters: int,
                 kernel_size: int,
                 strides=1,
        ...
        for i in range(seq_len):
            self.cell_list.append(SelfAttentionConvLSTM2DCell(filters=self.filters, 
    ...
    def call(self, inputs=None, states=None):
        ...
        if states is None:
            ...
            m = tf.zeros((self.batch_size, 112, 112, self.filters))
        else:
            c, h, m = states
        ...
        for i in range(self.seq_len):
            ...
            out, [h, c, m] = self.cell_list[i](x, [h, c, m])
            outputs.append(out)

        return tf.stack(outputs, axis=1), [h, c, m]
