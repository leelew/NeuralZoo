...
class BaseConvRNN(layers.Layer):
    def __init__(self,
                 num_filter,
                 b_h_w,
                 h2h_kernel=(3, 3),
...
#FIXME: Rewrite
class TrajGRU(BaseConvRNN,layers.Layer):
    def __init__(self,
                 num_filter,
                 b_h_w,
                 zoneout=0.0,
    ...
    def call(self, inputs=None, states=None):
        ...
        if states is None:
            states = tf.zeros((tf.shape(inputs)[1], self._state_height,
                               self._state_width, self._num_filter))
            states = tf.cast(states, 'float32')
        if inputs is not None:
            _, B, H, W, C = tf.shape(inputs)
            i2h = self.i2h(tf.reshape(inputs, (-1, H, W, C)))
            i2h = tf.reshape(
                i2h, (S, B, tf.shape(i2h)[1], tf.shape(i2h)[2],
                      tf.shape(i2h)[3]))
