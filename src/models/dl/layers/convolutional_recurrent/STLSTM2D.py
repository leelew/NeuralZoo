...
class predRNN(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for i in range(self.num_layers):
            self.cell_list.append(
                STLSTM2DCell(self.num_hidden[i], kernel_size=self.kernel_size))

            self.init_list.append(Dense(self.num_hidden[i]))
        self.conv_last = Conv2D(1, 1)
    ...
    def call(self, inputs, states=None):
        ...
        for i in range(self.num_layers):
            ...
            #initializer(shape=(self.h, self.w, self.num_hidden[i]))
            print(zeros.shape)
            #zeros = tf.zeros([self.b, self.h, self.w, self.num_hidden[i]])
            h_t.append(zeros)
            c_t.append(zeros)

        #memory = tf.zeros([self.b, self.h, self.w, self.num_hidden[0]])
        ...
        for t in range(self.t):
            ...
            out.append(self.conv_last(h_t[self.num_layers - 1]))
            print(self.conv_last(h_t[self.num_layers - 1]).shape)
